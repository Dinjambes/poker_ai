...
def new_game(
    ...
    logger.debug("Setup a new game")
    """Create a new game of short deck poker."""
    pot = Pot()
    players = [
        ShortDeckPokerPlayer(player_i=player_i, initial_chips=200, pot=pot)
...
class ShortDeckPokerState:
    ...
    def __init__(
        ...
        small_blind: int = 10,
        big_blind: int = 25,
        pickle_dir: str = "research/clustering/v2/data/",
    ...
    def apply_action(self, action_str: Optional[str]) -> ShortDeckPokerState:
        ...
        if action_str is None:
            ...
            action = new_state.current_player.allin()
            logger.debug("ALLIN!")
        ...
        while True:
            logger.debug("Player has made move, increment the player that is next. (_move_to_next_player)")
            logger.debug(f"Player before: {new_state.player_i}")
            new_state._move_to_next_player()
            logger.debug(f"Player after: {new_state.player_i}")
            ...
            if finished_betting and new_state.all_players_have_actioned:
                logger.debug("finished betting (All players have actioned), going to increment betting stage")
            ...
            if not new_state.current_player.is_active:
                ...
                if new_state._betting_stage in {"terminal", "show_down"}:
                    logger.debug("State was terminal/showdown, compute winners")
                    # Distribute winnings.
        ...
        logger.debug(f"Player before returning: {new_state.player_i}")
        return new_state
    ...
    def load_pickle_files(pickle_dir: str) -> Dict[str, Dict[Tuple[int, ...], str]]:
        ...
        # temp_dict : Dict[Tuple[int, ...], str] = {}
        # y = 0
        # for i in info_set_lut["pre_flop"]:
        #     temp_dict[tuple(i)] = y
        #     temp_dict[tuple(i[::-1])] = y
        #     y += 1
        # info_set_lut["pre_flop"] = temp_dict
        # print(str(temp_dict))

        return info_set_lut

    def _move_to_next_player(self):
        logger.debug("_move_to_next_player")
    ...
    def _reset_betting_round_state(self):
        logger.debug("_reset_betting_round_state")
    ...
    def _increment_stage(self):
        ...
        if self._betting_stage == "pre_flop":
            # Progress from private cards to the showdown.
            self._betting_stage = "show_down"
            logger.debug("Moving to showdown")
            if len(self._public_cards) == 5:
                logger.debug("public_cards = 5 so we take those")
                community_cards = self._public_cards[:5]
                self._poker_engine.table.community_cards += community_cards
            else:
                logger.debug("deal flop")
                self._poker_engine.table.dealer.deal_flop(self._table)
                logger.debug("deal turn")
                self._poker_engine.table.dealer.deal_turn(self._table)
                logger.debug("deal river")
                self._poker_engine.table.dealer.deal_river(self._table)
                logger.debug("community cards" + str(self._table.community_cards))
    ...
    def _info_set_builder(self, hole_cards=None, public_cards=None,
        ...
        #[134228773, 134224677],
        #(134228773, 134224677)
        # print('Before the getter: evalcards = ' + str(eval_cards) + ' type=' + str(type(eval_cards)))
        # print('infoset len = ' + str(self.info_set_lut.__len__()))
        # print('preflop infoset len = ' + str(self.info_set_lut[this_betting_stage]))
        try:
            cards_cluster = self.info_set_lut[this_betting_stage][eval_cards]
        except KeyError:
            logger.debug("COULD NOT FIND:" + str(eval_cards.__len__()) + " cards: " + str(eval_cards))
